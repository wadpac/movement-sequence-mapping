...
# inner function
accSummary2<- function (data, tri = "FALSE", axis = "NULL", spuriousDef = 20, 
                        ...
                        boutsize = 10, tolerance = "TRUE", returnbout = "TRUE") 
{
  ...
  epoch <- mean(c(as.numeric(difftime(strptime(data$TimeStamp[2], 
                ...
                na.rm = TRUE)
  if (epoch < 60) {
    data <- dataCollapser(data, TS = "TimeStamp", col = "counts", 
                          by = 60)
  }
  if (epoch > 60) {
  ...
  for (k in 1:length(dSplitPA)) {
    ...
    dsidPA <- data.frame(TimeStamp = dsiPA[, 1], counts = dsiPA[, 
                                                                2], nonwear = dsiPA[, 3], inPA2 = dsiPA[, 6])
    ...
    dsidPA$inbout[dsidPA$inbout == 1 & dsidPA$inboutUpLagb2 == 
                    0 & dsidPA$PALagUpb1 == 0 & dsidPA$inPA2 == 0] <- 0
    ...
    dsidPA$firstBout <- ifelse(dsidPA$first == 1 & dsidPA$inbout == 
                                 1 & dsidPA$wear == 1 & dsidPA$length >= boutsize, 
                               1, 0)
    mylistPA[[k]] <- dsidPA
    rm(dsidPA)
  }
  ...
  dfPA$inPABout <- ifelse(dfPA$inbout == 1 & dfPA$wear == 1, 
                          1, 0)
  ...
  daySumPA <- tapply(d4PA$paMinutes, format(dts2PA, format = "%Y-%m-%d"), 
                     sum)
  pasummary <- data.frame(Date = rownames(daySumPA), paMinutes = daySumPA, 
                          numberOfBoutsPA = boutSumPA)
  rownames(pasummary) <- NULL
  if (nrow(pasummary) == 0) {
    pasummary <- data.frame(format(dfPA$TimeStamp[1], format = "%Y-%m-%d"), 
