#' detect_bouts
#'
#' @description 'detect_bouts' identifies the bouts of which the length lie within a tolerance of 10 % below the lower threshold with a maximum of three consecutive minutes and updates the vector of bouts accordingly
#'
...
#' @param timethresholds A vector specifying the upper thresholds of the bout duration in minutes (Default : c(5, 10, 30, 60))
...
#' @param Nepoch_per_minute An integer defining the number of epochs per minute
#' @param bout_algorithm One of c("V1", "V2") defining the tolerance function used for the bout tolerance, where "V1" looks whether a pair of segments (based on intensity) is within a tolerance of 10 % using the function 'xinhui_bout_algorithm'; "V2" looks at the whole of segments to identify breaks in bouts within this tolerance (Default : "V2")
#'
...
detect_bouts <- function(bouts_values, bouts_lengths,
  ...
  getbout = function(x, boutduration, boutcriter = 0.8, epoch.size = 15, maximum.break.dur = 3) {
    ...
    # # only consider windows that at least start and end with value that meets criterion
    p = c(0, p, 0)
    if (epoch.size > 60) {
      epochs2check = 1
    } else {
    ...
    for (ii in 1:epochs2check) { # only check the first and last minutes of each bout
      # p are all epochs at the center of the windows that meet the bout criteria
      # we want to check the start and end of sequence of which centers whether
      # the the epoch half the bout length before and the epoch half the bout
      # length after this center meet the threshold criteria
      # So, we first zoom in on the edges of the sequence
      edges = which(diff(p) != 1)
      seq_start = p[edges + 1] # bout center starts
